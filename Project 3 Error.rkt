...
(define (m-eval exp env)
  (cond ((self-evaluating? exp) exp)
        ;; ========= QUESTION 4 =========
        ;Look up the first element in the list of variables as opposed to the entire list of previous values
        ((variable? exp) (car  (lookup-variable-value exp env)))
        ...
        ;; ======== QUESTION 5 ========
        ((current-env? exp) (environment-first-frame env)) ;;returns the current environment
        ((proc-env? exp) (procedure-environment (cadr exp)))
        ((application? exp)
...
;; ========= QUESTION 4 =========
;Cons the new value onto the list of previous variables
(define (set-variable-value! var val env)
  (let ((binding (find-in-environment var env)))
    (if binding
        (set-binding-value! binding (cons val (lookup-variable-value var env)))
        (error "Unbound variable -- SET" var))))

;; ========= QUESTION 4 =========
;Define a variable as a list instead of a single value
(define (define-variable! var val env)
  (let ((frame (environment-first-frame env)))
    (let ((binding (find-in-frame var frame)))
      (if binding
          (set-binding-value! binding (list val))
          (add-binding-to-frame!
           (make-binding var (list val))
           frame)))))
...
(define (primitive-procedures)
  (list (list 'car car)
        ...
        (list 'length length)
        ; ===== QUESTION 5 ======
        (list 'env-variables frame-variables)
...
;; ==== QUESTION 2 ====
;handle the special form 'and'

(define (and? exp) (tagged-list? exp 'and))

(define (and-clauses exps) (cdr exps))           
...
;; ==== QUESTION 3 ====
;Transform an 'until' expression into simpler known parts (ie, a let statement) to be evaluated
...
;identify an expression as an until
(define (until? exp) (tagged-list? exp 'until))

;; ========= QUESTION 4 =========
;Change the value associated with a variable to the previously set value by removing the first entry in the list of past values,
;or leaves the list alone if it has only one element

(define (eval-unset exp env)
  (let ((binding (find-in-environment (cadr exp) env)))
    (if binding
        (if (= (length (binding-value binding)) 1)
            (void)
            (set-binding-value! binding (cdr (lookup-variable-value (cadr exp) env))))
        (error "Unbound variable -- UNSET!" (cadr exp)))))

;identify unset! expressions
(define (unset? exp) (tagged-list? exp 'unset!))

;; ========= QUESTION 5 =========
;Add procedures that will allow inspection of frames and environments within the evaluator
...
(define (procedure-env proc)
  (if (lambda? proc)
      ...
      (error "Not a defined procedure -- PROCEDURE-ENV" proc)))
